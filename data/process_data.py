...
def load_data(messages_filepath, categories_filepath):
    ...
    if os.path.exists(categories_filepath):
        ...
        categories = pd.read_csv(
            categories_filepath + '.gz', compression='gzip')
...
def clean_data(df):
    """Clean data.
    Args:
        df: pd.DataFrame. Disaster messages and categories.
    Return:
        pd.DataFrame
    """
...
def save_data(df, database_filename, table_name='CleanedDataTable'):
    ...
    Args:
        df: pd.DataFrame. It contains disaster messages and categories that are cleaned.
    ...
    df.to_sql(table_name, engine, index=False,
              if_exists='replace', chunksize=600)
