...
def load_data(database_filepath, table_name='CleanedDataTable'):
    ...
    Returns:
       X: np.ndarray. Disaster messages.
       Y: np.ndarray. Disaster categories for each messages.
...
def save_stats(X, Y, category_names, vocabulary_stats_filepath, category_stats_filepath):
    ...
    Args;
        ...
        vocabulary_stats_filepath: String. Vocab stats is saved as pickle into this file.
        category_stats_filepath: String. Category stats is saved as pickle into this file.
    """
    # Check vocabulary
    ...
    vocabulary_df = vocabulary_df.sample(
        30, random_state=72).sort_values('counts', ascending=False)
    ...
    with open(vocabulary_stats_filepath, 'wb') as vocabulary_stats_file:
        pickle.dump((vocabulary_counts, vocabulary_words),
                    vocabulary_stats_file)
    ...
    with open(category_stats_filepath, 'wb') as category_stats_file:
        pickle.dump((category_counts, list(category_names)),
                    category_stats_file)
...
def build_model():
    ...
    cv = GridSearchCV(estimator=pipeline, param_grid=parameters,
                      cv=2, scoring='f1_weighted', verbose=3)
...
def evaluate_model(model, X_test, Y_test, category_names):
    ...
    Args:
        ...
        X_test: np.ndarray. Disaster messages.
        Y_test: np.ndarray. Disaster categories for each messages
        category_names: Disaster category names.
    """
...
def save_model(model, model_filepath):
    """Save model
    Args:
        model: sklearn.model_selection.GridSearchCV. Sklearn estimator.
...
def main():
    if len(sys.argv) == 5:
        database_filepath, model_filepath, vocabulary_filepath, category_filepath = sys.argv[
            1:]
        ...
        print('Saving stats...')
        save_stats(X, Y, category_names,
                   vocabulary_filepath, category_filepath)
